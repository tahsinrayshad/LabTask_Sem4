...
BEGIN
	-- Get the count of departments that were not affected by the update
	SELECT COUNT(*)
	INTO unaffected_departments_count
	FROM department
	WHERE budget <= 99999;
	
	-- Decrease the budget by 10% for departments with a budget greater than 99999
	UPDATE department
	SET budget = budget * 0.9
	WHERE budget > 99999;

	-- Output the count of unaffected departments
